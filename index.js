...
let botData = {
    ...
    // }
    pokemonSpawnChannels: {},  // { guildId: channelId }
    pokemonCache:         {},  // { key: data } — local API cache
    activeSpawns:         {},
    // activeSpawns[guildId] = {
    //   messageId, channelId, pokemon, shiny, spawnedAt, catchAttempts
    // }
    activeBattles:        {},
...
async function forceSaveNow() {
    try {
        ...
        isDirty = false;
        const kb = (JSON.stringify(getCleanData()).length / 1024).toFixed(2);
...
async function generateBattleImage(battle) {
    ...
    if (weather === 'sandstorm') {
        ...
        ctx.fillRect(0, 0, W, H);
        for (let i = 0; i < 30; i++) {
            ctx.fillStyle = `rgba(200,160,80,${Math.random() * 0.2})`;
            ctx.beginPath();
            ctx.ellipse(Math.random()*W, Math.random()*H, Math.random()*20+5, 3, Math.random()*Math.PI, 0, Math.PI*2);
            ctx.fill();
        }
    }
    if (weather === 'hail' || weather === 'snow') {
        for (let i = 0; i < 35; i++) {
            ctx.fillStyle = `rgba(200,230,255,${Math.random() * 0.4 + 0.1})`;
            ctx.beginPath();
    ...
    if (weather === 'sun') {
        ...
        sunGlow.addColorStop(0, 'rgba(255,200,50,0.35)');
    ...
    if (wInfo) {
        ...
        ctx.beginPath();
        ctx.roundRect(W/2 - 80, 5, 160, 24, 6);
        ctx.fill();
        ctx.fillStyle = '#ffffff';
        ctx.font      = 'bold 12px sans-serif';
    ...
    // ── Enemy sprite — LEFT side (HD official artwork) ──
    try {
    ...
    // ── Player sprite — RIGHT side (HD official artwork) ──
    try {
    ...
    function drawHPBar(x, y, current, max, name, level, status) {
        ...
        if (status) {
            ...
            ctx.beginPath();
            ctx.roundRect(x + 130, y + 6, 68, 16, 4);
            ctx.fill();
            ctx.fillStyle = '#ffffff';
        ...
        ctx.beginPath();
        ctx.roundRect(x + 28, y + 28, barW, barH, 5);
        ctx.fill();
        ...
        if (filled > 0) {
            ...
            ctx.beginPath();
            ctx.roundRect(x + 28, y + 28, filled, barH, 5);
            ctx.fill();
        }
    ...
    // ── Player HP bar — LEFT side bottom (opposite the sprite) ──
    drawHPBar(
        15, H - 88,
        p1.currentHp, p1.maxHp,
        formatPokeName(p1.pokemon.name), p1.pokemon.level,
        p1.statusEffect
    );
...
function checkAbilityImmunity(defenderFighter, moveType) {
    ...
    if (eff.trigger === 'superonly') return false; // handled in damage calc
    return false;
}
...
function applyWeatherEndOfTurn(battle, turnLog) {
    ...
    for (const side of ['player1', 'player2']) {
        ...
        if (w === 'sandstorm') {
            ...
            if (!types.some(t => ['rock','steel','ground'].includes(t)) &&
                eff.trigger !== 'weather' && !eff.immune) {
                const dmg = Math.max(1, Math.floor(fighter.maxHp / 16));
                fighter.currentHp = Math.max(0, fighter.currentHp - dmg);
        ...
        if ((w === 'rain' && (ability === 'rain-dish' || ability === 'dry-skin')) ||
...
async function executeTurn(battleId, channel) {
    ...
    for (const who of order) {
        ...
        if (checkAbilityImmunity(defender, moveData.type)) {
            ...
            // Flash Fire etc — boost on immunity
...
// ──────────────────────────────────────────
    ...
    if (command === 'pokedex') {
        ...
        const embed = new EmbedBuilder()
            .setColor(0x3498DB)
            .setTitle(`#${data.id} — ${formatPokeName(data.name)}`)
            ...
            .setFooter({ text: 'SOLDIER² Pokémon • Data from PokéAPI' })
            .setTimestamp();
    ...
    if (command === 'givepokemon') {
        ...
        const shiny = isShiny ? true : false;
        const entry = buildPokemonEntry(data, shiny, level);
        addPokemonToUser(target.id, entry);

        const embed = new EmbedBuilder()
            .setColor(shiny ? 0xFFD700 : 0x24c718)
            .setTitle(`${shiny ? '✨ Shiny ' : ''}Pokémon Given!`)
            .setDescription(`<@${target.id}> received **${shiny ? '✨ ' : ''}${formatPokeName(data.name)}**!`)
            .setThumbnail(shiny ? data.spriteShiny : data.sprite)
            .addFields(
            ...
            .setFooter({ text: `Given by ${message.author.tag} • SOLDIER² Pokémon` })
            .setTimestamp();
    ...
    if (command === 'party') {
        ...
        if (!ud.party.length)
            return reply(`❌ ${targetUser.id === uid ? 'You have' : `<@${targetUser.id}> has`} no Pokémon in their party.`);

        const partyPokemon = ud.party.map(idx => ud.collection[idx]).filter(Boolean);

        const embed = new EmbedBuilder()
            .setColor(0x3498DB)
            .setTitle(`🎮 ${targetUser.username}'s Battle Party`)
            .setThumbnail(targetUser.displayAvatarURL({ dynamic: true }))
            .setFooter({ text: `${partyPokemon.length}/6 slots filled • SOLDIER² Pokémon` })
            .setTimestamp();
        ...
        for (let i = 0; i < partyPokemon.length; i++) {
            ...
            embed.addFields({
                name:  `${i + 1}. ${p.shiny ? '✨ ' : ''}${formatPokeName(p.name)} — Lv.${p.level}`,
                value:
                    `HP: ${bar}\n` +
                    `Type: **${p.types.map(t => formatPokeName(t)).join(' / ')}**\n` +
                    `XP: \`${p.xp}/${xpNeeded}\`\n` +
                    `Moves: ${p.moves.map(m => `\`${formatPokeName(m)}\``).join(', ')}`,
                inline: false,
            });
        }
    ...
    if (command === 'collection') {
        ...
        if (!ud.collection.length)
            return reply(`❌ ${targetUser.id === uid ? 'You have' : `<@${targetUser.id}> has`} no Pokémon yet.`);
        ...
        const lines = slice.map((p, i) => {
            ...
            return (
                `**${start + i + 1}.** ${p.shiny ? '✨ ' : ''}${formatPokeName(p.name)} — ` +
                `Lv.${p.level} | ${p.types.map(t => formatPokeName(t)).join('/')}` +
                `${inParty ? ' ⚔️' : ''}`
            );
        });
        ...
        const embed = new EmbedBuilder()
            ...
            .setDescription(lines.join('\n'))
            .setThumbnail(targetUser.displayAvatarURL({ dynamic: true }))
            .setFooter({ text: `Page ${safePage}/${totalPages} • ${ud.collection.length} total • ⚔️ = in party • SOLDIER² Pokémon` })
            .setTimestamp();
    ...
    if (command === 'inspect') {
        ...
        if (isNaN(num) || num < 1 || num > ud.collection.length)
            return reply(`❌ Usage: \`×inspect <number>\` — use \`×collection\` to see your numbers.`);
        ...
        const embed = new EmbedBuilder()
            ...
            .setTitle(`${p.shiny ? '✨ ' : ''}${formatPokeName(p.name)} — Lv.${p.level}`)
            ...
            .setFooter({ text: `Collection #${num} • SOLDIER² Pokémon` })
            .setTimestamp();
    ...
    if (command === 'release') {
        ...
        if (isNaN(num) || num < 1 || num > ud.collection.length)
            return reply(`❌ Usage: \`×release <number>\` — use \`×collection\` to see your numbers.`);
        ...
        return reply({ embeds: [
            new EmbedBuilder()
                ...
                .setDescription(
                    `You released **${pkm.shiny ? '✨ ' : ''}${formatPokeName(pkm.name)}** (Lv.${pkm.level}).\n` +
    ...
    if (command === 'pokeparty') {
        ...
        if (action === 'add') {
            ...
            if (isNaN(num) || num < 1 || num > ud.collection.length)
                return reply(`❌ Usage: \`×pokeparty add <number>\``);

            const idx = num - 1;
            const pkm = ud.collection[idx];
            if (ud.party.includes(idx))
                return reply(`❌ **${formatPokeName(pkm.name)}** is already in your party.`);
            if (ud.party.length >= 6)
                return reply('❌ Your party is full (6/6). Remove one first.');
        ...
        if (action === 'remove') {
            const num = parseInt(args[1]);
            if (isNaN(num) || num < 1 || num > ud.collection.length)
                return reply(`❌ Usage: \`×pokeparty remove <number>\``);

            const idx = num - 1;
            const pkm = ud.collection[idx];
            if (!ud.party.includes(idx))
                return reply(`❌ **${formatPokeName(pkm.name)}** is not in your party.`);
        ...
        if (action === 'swap') {
            ...
            if (
                ...
                slot1 < 0 || slot1 >= ud.party.length ||
                slot2 < 0 || slot2 >= ud.party.length
            ) return reply(`❌ Usage: \`×pokeparty swap <slot1> <slot2>\` — slots are 1 to ${ud.party.length}`);

            const temp       = ud.party[slot1];
            ud.party[slot1]  = ud.party[slot2];
            ud.party[slot2]  = temp;
            markDirty(); scheduleSave();

            const pkm1 = ud.collection[ud.party[slot1]];
            const pkm2 = ud.collection[ud.party[slot2]];
            return reply(
                `✅ Swapped slot **${slot1 + 1}** (${formatPokeName(pkm2.name)}) ` +
                `and slot **${slot2 + 1}** (${formatPokeName(pkm1.name)}).`
            );
        }
    ...
    if (command === 'setspawnchannel') {
        if (!canSetSpawnChannel(gid, uid))
            return reply('❌ You need at least Enlisted rank to set the spawn channel.');
        ...
        if (!targetChannel || targetChannel.type !== 0)
            return reply('❌ Please mention a valid text channel. Example: `×setspawnchannel #wild-pokemon`');
        ...
        return reply({ embeds: [
            new EmbedBuilder()
                .setColor(0x24c718)
                .setTitle('✅ Spawn Channel Set')
                .setDescription(
                    `Wild Pokémon will now appear in <#${targetChannel.id}>.\n` +
                    `First spawn in **${SPAWN_MIN_MINUTES}–${SPAWN_MAX_MINUTES} minutes**.`
                )
                .setFooter({ text: `Set by ${message.author.tag} • SOLDIER² Pokémon` })
                .setTimestamp()
        ]});
    }
    ...
    if (command === 'spawnpokemon') {
        ...
        if (!channelId)
            return reply('❌ No spawn channel set. Use `×setspawnchannel #channel` first.');
    ...
    if (command === 'resetpokemon') {
        ...
        return reply({ embeds: [
            new EmbedBuilder()
                .setColor(0xE74C3C)
                .setTitle('🗑️ Pokémon Collection Reset')
    ...
    if (command === 'battle') {
        ...
        if (!ud.party.length || !ud.collection[ud.party[0]])
            return reply('❌ You have no Pokémon in your party. Use `×pokeparty add <number>` to add one.');

        const mode      = args[0]?.toLowerCase();
        const playerPkm = ud.collection[ud.party[0]];

        // Check already in battle
        const alreadyIn = Object.values(botData.activeBattles || {})
            .find(b => b.player1.userId === uid || b.player2.userId === uid);
        ...
        if (!mode || mode === 'pve') {
            ...
            const battle = {
                ...
                player1: {
                    userId:        uid,
                    pokemon:       playerPkm,
                    currentHp:     playerPkm.stats.hp,
                    maxHp:         playerPkm.stats.hp,
                    statusEffect:  null,
                    sleepTurns:    0,
                    confusionTurns: 0,
                    atkMod:        1,
                ...
                player2: {
                    userId:        'BOT',
                    pokemon:       wildPkm,
                    currentHp:     wildPkm.stats.hp,
                    maxHp:         wildPkm.stats.hp,
                    statusEffect:  null,
                    sleepTurns:    0,
                    confusionTurns: 0,
                    atkMod:        1,
        ...
        if (mode === 'pvp') {
            ...
            if (!opponentUd.party.length || !opponentUd.collection[opponentUd.party[0]])
                return reply(`❌ <@${opponent.id}> has no Pokémon in their party.`);

            const existingBattle = Object.values(botData.activeBattles || {}).find(b =>
                b.player1.userId === opponent.id || b.player2.userId === opponent.id
            );
            ...
            const challengeEmbed = new EmbedBuilder()
                ...
                .setDescription(
                    `<@${uid}> challenged <@${opponent.id}> to a battle!\n\n` +
                ...
                .setFooter({ text: 'Challenge expires in 60 seconds • SOLDIER² Pokémon' })
                .setTimestamp();
            ...
            collector.on('collect', async (r) => {
                ...
                if (r.emoji.name === '❌') {
                    return challengeMsg.edit({ embeds: [
                        new EmbedBuilder()
                            .setColor(0xE74C3C)
                            .setTitle('❌ Battle Declined')
                            .setDescription(`<@${opponent.id}> declined the battle.`)
                            .setTimestamp()
                    ]}).catch(() => {});
                }
    ...
    // ──────────────────────────────────────────
    // ×switch <slot> — voluntarily switch pokemon mid-battle
    // ──────────────────────────────────────────
    if (command === 'switch') {
        const battleEntry = Object.entries(botData.activeBattles || {})
            .find(([, b]) => b.player1.userId === uid || b.player2.userId === uid);
        ...
        if (isNaN(slotArg) || slotArg < 0 || slotArg >= ud.party.length)
            return reply(`❌ Usage: \`×switch <party slot>\` — slots 1 to ${ud.party.length}`);
        ...
        // Switching costs your turn — set move to null so no attack this turn
        battle.player1.pokemon   = chosen;
        ...
        // Apply Intimidate if bot has it
        applyAbilityOnEntry(battle.player2, battle.player1, switchLog);

        // If PvE, bot still attacks since player used turn to switch
        if (battle.type === 'pve') {
            battle.p1Move = null; // skip player attack
            await executeTurn(battleId, message.channel);
        } else {
            // PvP — update embed and wait for both moves
            const imgBuf  = await generateBattleImage(battle).catch(() => null);
            ...
            if (battleMsg) {
                await battleMsg.edit({
                    embeds: [buildBattleEmbed(battle, switchLog, imgFile)],
                    files:  imgFile ? [imgFile] : [],
                }).catch(() => {});
            }
        }
    ...
    if (command === 'trade') {
        ...
        if (!target || target.bot || target.id === uid)
            return reply('❌ Usage: `×trade @user <collection number>`');
        ...
        const ud  = getUserPokemon(uid);
        const idx = numArg - 1;
        if (idx < 0 || idx >= ud.collection.length || !ud.collection[idx])
            return reply(`❌ No Pokémon at position ${numArg}. Check \`×collection\`.`);
        ...
        const tradeEmbed = new EmbedBuilder()
            ...
            .setDescription(
                `<@${uid}> wants to trade their **${pkm.shiny ? '✨ ' : ''}${formatPokeName(pkm.name)}** (Lv.${pkm.level}) with <@${target.id}>!\n\n` +
                `<@${target.id}> — React ✅ to **accept** or ❌ to **decline** *(60 seconds)*`
            )
            .setThumbnail(pkm.sprite)
            .addFields(
                { name: '🔷 Type',  value: pkm.types.map(t => formatPokeName(t)).join(' / '), inline: true },
                { name: '⭐ Level', value: `${pkm.level}`,                                    inline: true },
                { name: '✨ Shiny', value: pkm.shiny ? 'YES 🌟' : 'No',                       inline: true },
            )
            .setFooter({ text: 'SOLDIER² Pokémon Trade' })
            .setTimestamp();

        const tradeMsg = await message.channel.send({ content: `<@${target.id}>`, embeds: [tradeEmbed] });
        await tradeMsg.react('✅').catch(() => {});
        await tradeMsg.react('❌').catch(() => {});
        ...
        collector.on('collect', async (reaction) => {
            if (reaction.emoji.name === '❌') {
                await tradeMsg.edit({ embeds: [new EmbedBuilder()
                    ...
                    .setDescription(`<@${target.id}> declined the trade.`)
                    .setFooter({ text: 'SOLDIER² Pokémon Trade' })
                ]}).catch(() => {});
                return;
            }

            // Ask target what they want to offer
            const targetUd = getUserPokemon(target.id);
            if (!targetUd.collection.length)
                return tradeMsg.edit({ embeds: [new EmbedBuilder()
                    .setColor(0xE74C3C)
                    .setTitle('❌ Trade Failed')
                    .setDescription(`<@${target.id}> has no Pokémon to trade!`)
                    .setFooter({ text: 'SOLDIER² Pokémon Trade' })
                ]}).catch(() => {});

            await tradeMsg.edit({ embeds: [new EmbedBuilder()
                .setColor(0xFF6900)
                .setTitle('🔄 Trade Accepted — Pick Your Pokémon')
                .setDescription(
                    `<@${target.id}> — reply with \`×tradeoffer <number>\` within 60 seconds to pick which Pokémon you offer.\n` +
                    `Use \`×collection\` to see your Pokémon numbers.`
                )
                .setFooter({ text: 'SOLDIER² Pokémon Trade' })
            ]}).catch(() => {});

            // Wait for tradeoffer
            const offerFilter  = m => m.author.id === target.id &&
                m.content.toLowerCase().startsWith(`${prefix}tradeoffer`);
            ...
            offerCollect.on('collect', async (offerMsg) => {
                ...
                if (targetIdx < 0 || targetIdx >= targetUd.collection.length || !targetUd.collection[targetIdx]) {
                    return message.channel.send(`❌ <@${target.id}> — invalid Pokémon number.`);
                }
                ...
                const confirmEmbed = new EmbedBuilder()
                    ...
                    .setDescription(
                        `**${formatPokeName(pkm.name)}** (Lv.${pkm.level}) ↔️ **${formatPokeName(targetPkm.name)}** (Lv.${targetPkm.level})\n\n` +
                        `<@${uid}> — React ✅ to confirm or ❌ to cancel.`
                    )
                    .setFooter({ text: 'SOLDIER² Pokémon Trade' });
                ...
                cc.on('collect', async (r) => {
                    if (r.emoji.name === '❌') {
                        await confirmMsg.edit({ embeds: [new EmbedBuilder()
                            .setColor(0x95A5A6).setTitle('❌ Trade Cancelled')
                            .setFooter({ text: 'SOLDIER² Pokémon Trade' })
                        ]}).catch(() => {});
                        return;
                    }
                    ...
                    // Execute trade
                    const senderPkmCopy = { ...pkm };
                    const targetPkmCopy = { ...targetPkm };

                    ud.collection[idx]            = targetPkmCopy;
                    targetUd.collection[targetIdx] = senderPkmCopy;
                    markDirty(); scheduleSave();
                    ...
                    await confirmMsg.edit({ embeds: [new EmbedBuilder()
                        ...
                        .setDescription(
                            `<@${uid}> received **${formatPokeName(targetPkm.name)}**!\n` +
                            `<@${target.id}> received **${formatPokeName(pkm.name)}**!`
                        )
                        .setFooter({ text: 'SOLDIER² Pokémon Trade' })
                        .setTimestamp()
                    ]}).catch(() => {});
        ...
        collector.on('end', (c) => {
            if (!c.size) tradeMsg.edit({ embeds: [new EmbedBuilder()
                .setColor(0x95A5A6).setTitle('⏰ Trade Expired')
                .setDescription(`<@${target.id}> did not respond in time.`)
                .setFooter({ text: 'SOLDIER² Pokémon Trade' })
            ]}).catch(() => {});
        });
        return;
    ...
    // ──────────────────────────────────────────
    // ×pokeleaderboard [catches/wins/shinies]
    // ──────────────────────────────────────────
    if (command === 'pokeleaderboard') {
        const type    = args[0]?.toLowerCase() || 'catches';
        const validLb = ['catches', 'wins', 'shinies', 'level'];
        if (!validLb.includes(type))
            return reply(`❌ Usage: \`×pokeleaderboard catches/wins/shinies/level\``);
        ...
        const desc   = top.map((e, i) =>
            `${medals[i] || `**${i+1}.**`} <@${e.userId}> — **${e.value.toLocaleString()}** ${
                type === 'catches' ? 'caught' : type === 'wins' ? 'wins' : type === 'shinies' ? '✨ shinies' : 'highest lv.'
            }`
        ).join('\n');
        ...
        await message.channel.send({ embeds: [new EmbedBuilder()
            ...
            .setDescription(desc)
            .setFooter({ text: 'SOLDIER² Pokémon Leaderboard' })
            .setTimestamp()
        ]});
        return;
    }
    ...
    if (command === 'dexcompletion') {
        ...
        await message.channel.send({ embeds: [new EmbedBuilder()
            .setColor(0xFF6900)
            .setTitle(`📖 ${target.username}'s Pokédex Completion`)
            .addFields(
            ...
            .setThumbnail(target.displayAvatarURL({ dynamic: true }))
            .setFooter({ text: 'SOLDIER² Pokédex' })
            .setTimestamp()
        ]});
        return;
    }
    ...
    if (command === 'forfeit') {
        ...
        // Record win for opponent if PvP
        if (battle.type === 'pvp') {
        ...
        if (channel) {
            await channel.send({ embeds: [
                new EmbedBuilder()
                    ...
                    .setDescription(`<@${uid}> forfeited the battle!`)
                    .setTimestamp()
                    .setFooter({ text: 'SOLDIER² Pokémon Battle' })
            ]}).catch(() => {});
            ...
            if (battle.battleMsgId) {
                const battleMsg = await channel.messages.fetch(battle.battleMsgId).catch(() => null);
                if (battleMsg) {
                    await battleMsg.edit({ embeds: [
                        buildBattleEmbed(battle)
                            .setTitle('🏳️ Battle Ended — Forfeit')
                            .setColor(0xE74C3C)
                    ]}).catch(() => {});
                }
            }
    ...
    if (command === 'ping') {
        ...
        if (apiLatency < 100)      { color = 0x2ECC71; label = 'Excellent'; indicator = '🟢'; }
        else if (apiLatency < 200) { color = 0xF1C40F; label = 'Moderate';  indicator = '🟡'; }
        else                       { color = 0xE74C3C; label = 'Poor';      indicator = '🔴'; }
    ...
    if (command === 'cleanjson') {
        ...
        botData.xpCooldowns = {}; botData.commandLog = {}; botData.dutyStatus = {}; botData.antiraidSnapshot = {};
        const result = await forceSaveNow();
...
// ×pokehelp — Pokémon system help
    ...
    if (command === 'pokehelp') {
        ...
        const embed3 = new EmbedBuilder()
            ...
            .setDescription(
                `**━━━ STAFF COMMANDS ━━━**\n` +
                `• \`${prefix}givepokemon @user <name> [shiny] [level]\` — Give a Pokémon *(Officers+)*\n` +
    ...
    if (command === 'staffhelp') {
        ...
        const embed2 = new EmbedBuilder()
            ...
            .setDescription(
                ...
                `• \`${prefix}ping\` — Check bot latency with color indicator\n` +
                `• \`${prefix}forcesave\` — Force save all data to JSONBin *(Owner/Generals/Officers)*\n` +
